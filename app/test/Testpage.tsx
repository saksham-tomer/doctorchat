...
export default function Page({
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setSendChatState((prev) => ({
      ...prev,
      newValue: (e.target as HTMLInputElement).value, // Cast event target to HTMLInputElement
    }));
                  ...
                  <div className="flex flex-col items-center relative">
                    {Object.keys(sendChatState).map((send) => {
                      return (
                        <div
                          key={send}
                          className="mr-auto ml-2 bg-indigo-200 rounded-lg px-2 py-1 "
                        >
                          {send}
                        </div>
                      );
                    })}
                    <div className="ml-auto mr-2 bg-indigo-300 rounded-lg px-2 py-1 ">
                      {}
                    </div>
                    <form
                      className="flex items-center justify-center absolute transform translate-y-32"
