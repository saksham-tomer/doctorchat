...
function Page() {
  ...
  // const memoiseDoctors = useMemo(async()=>{
...
//     const data = await memoizedDoctors;
//     setDoctors(data);
//   }
//   fetchData();
// }, [memoizedDoctors])
  ...
  return (
    <div className="mt-12 relative">
      ...
      <div>
        <div className="flex flex-row items-center mx-2 mt-6 justify-between">
          ...
          <div className="flex gap-4 shadow-xl py-2 px-4 bg-gray-50 flex-row ring-1 ring-gray-100 rounded-lg mr-4">
            <div onClick={()=>toggleLayout((prev)=>prev = "card")} className="p-2 rounded-xl shadow-lg cursor-pointer bg-gray-300 hover:bg-gray-400">
              <Image src={'/layout.png'} alt="small" width={18} height={18} />
            </div>
            <div onClick={()=>toggleLayout((prev)=>prev = "stack")}className="p-2 rounded-xl cursor-pointer shadow-lg bg-gray-300 hover:bg-gray-400">
      ...
      <div className="mx-2 mt-4 flex flex-col gap-4 items-center">
        {layout === 'stack' && <div className="flex flex-col gap-4">
          {
...
{layout === 'card' && <div className="relative"> <div  ref={sliderRef} className="flex scrollbar-hide min-h-72 items-center flex-row overflow-y-hidden max-w-72 rounded-lg gap-8  md:grid md:gap-6 md:grid-cols-3 md:grid-rows-2">{
  ...
  <button onClick={forward} className="absolute  rounded-full bg-indigo-300 hover:bg-indigo-400 text-white px-2 shadow-lg py-2 transform translate-x-64 translate-y-3">
    <Image src={'/arrowright1.png'} alt="arrow" width={30} height={30} />
  </button>
   <button onClick={backward} className="absolute  rounded-full bg-indigo-300 hover:bg-indigo-400 text-white px-2 py-2 transform shadow-md -translate-x-8 translate-y-3">
    <Image src={'/arrowleft.png'} alt="arrow" width={30} height={30} />  
   </button>
  </div>
  </div>}
      </div>
