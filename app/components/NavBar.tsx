...
export default function NavBar({SigninButton}: {SigninButton: any}) {
  ...
  return (
    <div className="fixed z-10 top-0 min-w-full overflow-hidden">
      <div className="relative">
        <div className="fixed min-w-full min-h-12 flex flex-row items-center justify-between bg-white pt-0">
          ...
          <div onClick={()=>(setToggle(prev=>!prev))} className="pr-4 md:hidden hover:cursor-pointer flex items-center">{
            toggle ? "" : <Image className="bg-gray-200 p-2 rounded-full cursor-pointer hover:bg-gray-300" src={'/menu.png'} alt="menu" height={38} width={38} />
          }</div>
          <ul className="hidden md:flex lg:flex felx-row gap-6 text-gray-500 font-medium">
            <Link href={'/'}>
            <li className="bg-slate-100 px-4 rounded-lg py-1 shadow-md">Home</li>
            </Link>
            <Link href={'/help'}>
            <li className="bg-slate-100 px-6 rounded-lg py-1 shadow-md">Help</li>
            </Link>
          </ul>
          <div className="hidden md:flex  pr-4  flex-row gap-4 items-center">
            {SigninButton ? <SigninButton /> :
            <div>
            <Link className="text-gray-500 font-bold" href={'/auth/sign-up'}>
              Register
            </Link>
            <motion.div className="rounded-2xl bg-blue-600 shadow-lg shadow-blue-300 hover:cursor-pointer hover:translate-y-1 font-semibold text-white px-4 py-1"><Link href={'/auth/log-in'} >
              Login</Link>
              </motion.div>
              </div>
}
            ...
            >
              <div className="mt-8 ">
                ...
                <div className="bg-gradient-to-bl from-blue-200 to-indigo-300 p-2 mr-4 mb-8 rounded-2xl ring-1 ring-teal-200 shadow-lg flex flex-col items-center justify-center">
                  <Image className="rounded-full shadow-lg" src={session?.user.image} alt="image" height={40} width={40} />
                  <Greeting name={session?.user.name} />
                </div> : <div onClick={()=>signIn(undefined,{callbackUrl:'/patients'})} className="px-4 py-1 rounded-lg shadow-lg bg-yellow-400 ring-1 ring-orange-300 font-bold mr-4 mb-4 text-white">Login</div>
                }
                <div className="flex items-center flex-row gap-24 text-gray-700 text-sm font-bold pb-6 pl-2">Menu
                 <span className="ml-auto bg-gray-200 hover:bg-gray-300 p-2 rounded-full ring-1 ring-gray-300 mr-6 hover:cursor-pointer" onClick={()=>(setToggle((prev)=>!prev))}>
                ...
                className="text-base text-gray-600 font-medium flex flex-col gap-6 ">
                  ...
                  <li onClick={redirectPatient} className="flex gap-4 items-center  hover:cursor-pointer  flex-row">
                    <Image
                      src="/activity.gif"
                      width={30}
                      height={30}
                  ...
                  <li className="flex gap-4  hover:cursor-pointer items-center  flex-row">
                    <Link className="flex gap-4  hover:cursor-pointer items-center  flex-row" href={`/account/${session?.user.name}`}>
                    <Image
                      src="/settings.gif"
                      width={30}
                      height={30}
                  ...
                  {session && 
                   <li onClick={()=>signOut()} className="flex gap-4 bg-red-500 max-w-36 px-2 py-0 text-white ml-1 rounded-2xl ring-1 ring-red-600 shadow-xl hover:cursor-pointer items-center  flex-row">
                    <Image
                      src="/exit.png"
                      width={20}
                      height={20}
